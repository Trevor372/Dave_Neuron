...
Dave_Neuron::Dave_Neuron(void) : DefaultGUIModel("Dave_Neuron", ::vars, ::num_vars) {
    setWhatsThis(
    "<p><b>Dave_Neuron:</b><br>This module simulates a neuron with an A Current.</p>");
...
Dave_Neuron::~Dave_Neuron(void) {}

void Dave_Neuron::execute(void) {
...
void Dave_Neuron::update(DefaultGUIModel::update_flags_t flag) {
    switch (flag) {
        case INIT:
...
void Dave_Neuron::initParameters() {
    V0 = -55.038; // mV
    G_Na_max = 0.24;
    G_K_max = 0.011;
    G_L = 0.0024;
...
void Dave_Neuron::solve(double dt, double *y) {
    double dydt[7];
    derivs(y, dydt);
...
void Dave_Neuron::derivs(double *y, double *dydt) {
